...
<div class="service-card">
    ...
    <form action="{{ route('admin.serviceStore') }}" method="POST" enctype="multipart/form-data">
        @csrf
        <div class="form-grid">
            <input type="text" name="nom_service" placeholder="Titre du service" required value="{{ old('nom_service') }}">
            <textarea name="description" placeholder="Description">{{ old('description') }}</textarea>
            <input type="text" name="lien" placeholder="Lien du service" required value="{{ old('lien') }}">
...
<div class="service-list">
    @forelse($servicesProposes as $service)
        <div class="card">
            <img src="{{ $service->image ?? asset('medias/default-service.jpg') }}" alt="Service">
            <div class="card-body">
                <h5 class="card-title">{{ $service->nom_service }}</h5>
                <p class="card-text">{{ $service->description }}</p>
                <form action="{{ route('admin.serviceDestroy', $service->id) }}" method="POST" onsubmit="return confirm('Voulez-vous vraiment supprimer ce service ?');">
                    @csrf
                    @method('DELETE')
